...
interface Context{
	...
	public function resetEmergency();
...
class SafeFrame implements Context{
	...
	public function setClock($time){
		if(! ($this->state instanceof EmergencyState)){	//緊急時の場合は、状態変更させない
			echo '現在時刻' . $time . PHP_EOL;
			$this->time = $time;						//緊急時から戻れるようにするため、$timeを覚えておくようにする
	...
	public function setEmergency(){
		echo '緊急時に遷移' . PHP_EOL;
		$this->changeState(EmergencyState::getInstance());
	}

	public function resetEmergency(){
		echo '緊急時解除' . PHP_EOL;
		$this->state->doClock($this, $this->time);
	}
...
class EmergencyState implements State{
	const useMessage   = "非常時の金庫使用";
	const alarmMessage = "非常ベル";
	const phoneMessage = "非常時通話";
...
actionFacade($safeFrame);
$safeFrame->setEmergency();
actionFacade($safeFrame);
$safeFrame->resetEmergency();
