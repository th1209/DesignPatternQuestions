「状態毎に複数の処理が切り替わる」というケースに出くわした場合→Stateパターンを疑ってみよう
時間で処理を切り替えたいとかの場合でも、Stateパターンならラクラク

状態毎に分けるので複雑にならない
メソッド内の条件分岐とおさらば
状態毎で同じ処理は、スーパークラスで汎化可能

状態を追加するのはカンタン(各Stateクラスを微妙にいじらねばならないが)。

状態に依存した処理を追加するのは難しい(各Stateクラスに、状態に依存した処理を追加する必要が出てくる)
